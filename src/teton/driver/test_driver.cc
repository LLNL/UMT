...
#pragma omp target
      ...
      if (myRank == 0)
      {
         ...
         std::cout << "(" << num_corners << " spatial elements * " << num_angles << " directions (angles) * "
                   << num_groups << " energy groups)" << std::endl;
      ...
      if (myRank == 0)
      {
         ...
         if (benchmarkProblem > -1)
         {
            ...
            {
               std::cerr << "Benchmark problem " << benchmarkProblem
                         << " does not have reference data to compare against." << std::endl;
            }
            else
            {
               std::cerr << "Benchmark problem " << benchmarkProblem
      ...
      if (useGPU == 1 && useUmpire > 0)
      {
         ...
         if (useUmpire > 1)
         {
            ...
            if (myRank == 0)
               print_bytes_as_gb("Teton driver: Thread safe device pool size: ",
                                 thread_safe_device_pool.getActualSize());
