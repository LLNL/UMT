...
!***********************************************************************
   ...
   subroutine SweepGreyUCBrzNEW(setID, Angle, PsiB)
...
!$omp  parallel do private(ii, zone, nCorner, c0)  &
!$omp& shared(angle0, ndoneZ, Angle, setID, omega)  &
!$omp& shared(fac, quadTauW1, quadTauW2, StartingDirection) schedule(static)
...
!  Set the "half-angle" angular intensity (PsiM) for the next angle
   ...
   return
   end subroutine SweepGreyUCBrzNEW
...
!  Set the "half-angle" angular intensity (PsiM) for the next angle
   ...
   if ( StartingDirection ) then
     ...
     quadTauW1 = one/quadtau
     quadTauW2 = (one - quadtau)/quadtau
