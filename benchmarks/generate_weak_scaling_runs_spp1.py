...
mem_upper_bound = node_memory * node_memory_target * (1 + memory_tolerance) * 1024 * 1024 * 1024 / max_ranks
...
for r in range(1,1000):
   ...
   for p in range(1,max_ranks+1):
      mem_per_rank = totalMem / p
      if mem_per_rank >= mem_lower_bound and mem_per_rank <= mem_upper_bound:
         DOFsPerRank = int(totalDOFs / p)
         if 8*p < zones:
            print(f"srun -N1-1 -n {p} --exclusive ./test_driver -c 1 -b 1 -r 1 -R {r} -o . -i unstructBox3D.mesh >& run.spp1.{p}_ranks.{DOFsPerRank}_dofs_per_rank.{zones}_zones.{totalDOFs}_total_dofs.log")
