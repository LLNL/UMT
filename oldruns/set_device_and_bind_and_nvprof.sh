...
if [ $OMPI_COMM_WORLD_RANK = 0 ]; then
    ...
    exec nvprof -s -f -o ${num_nodes}nodes.%q{OMPI_COMM_WORLD_RANK}.nvprof $@
    #exec nvprof -s --kernels ::sweep:10 --analysis-metrics -f -o ${num_nodes}nodes.%q{OMPI_COMM_WORLD_RANK}.nvprof $@
else
    exec $@
fi
